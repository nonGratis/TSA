...
def run_pipeline(df: pd.DataFrame, config: Dict) -> pd.DataFrame:
    ...
    (включає обчислення NIS та approximate NEES для позиції)
    """
    ...
    innovation_covs = np.full(n_total, np.nan)  # Для діагностики
    nis_vals = np.full(n_total, np.nan)        # NIS per update
    nees_pos_vals = np.full(n_total, np.nan)   # Approximate NEES for position (uses measurement as proxy)

    # Перший крок
    kf_x[0] = kf.get_position()
    kf_v[0] = kf.get_velocity()
    kf_p_var[0] = float(kf.P[0, 0])
    ...
    for i in range(1, n_total):
        ...
        # Update step
        if should_update:
            # Compute NIS using innovation and innovation covariance BEFORE update
            nis = KalmanFilter.compute_nis(residual, innovation_cov)
            nis_vals[i] = nis
            ...
            # After update compute approximate NEES for position using measurement as proxy.
            # NOTE: this is an approximation; true NEES requires true state.
            try:
                nees_pos = kf.compute_nees_position(measurement)
            except Exception:
                nees_pos = np.nan
            nees_pos_vals[i] = nees_pos

        # Зберігаємо результати
        kf_x[i] = kf.get_position()
        kf_v[i] = kf.get_velocity()
        kf_p_var[i] = float(kf.P[0, 0])
        if state_dim >= 3 and kf_a is not None:
    ...
    valid_mask = ~is_imputed
    valid_residuals = residuals[valid_mask]
    valid_innov_covs = innovation_covs[valid_mask]
    valid_nis = nis_vals[valid_mask]
    valid_nees = nees_pos_vals[valid_mask]

    if np.count_nonzero(~np.isnan(valid_residuals)) > 1:
        empirical_var_innov = float(np.nanvar(valid_residuals, ddof=1))
        mean_S_k = float(np.nanmean(valid_innov_covs))
    ...
    # NIS summary (для одномірного виміру NIS має мат. сподівання ≈ dof (1))
    if np.count_nonzero(~np.isnan(valid_nis)) > 0:
        mean_nis = float(np.nanmean(valid_nis))
        median_nis = float(np.nanmedian(valid_nis))
        prop_nan = float(np.isnan(valid_nis).sum()) / max(1, len(valid_nis))
        print(f"\n    NIS (std innovations) - mean: {mean_nis:.3f}, median: {median_nis:.3f}, NaN share: {prop_nan:.2%}")

    # NEES summary (approx for position)
    if np.count_nonzero(~np.isnan(valid_nees)) > 0:
        mean_nees = float(np.nanmean(valid_nees))
        median_nees = float(np.nanmedian(valid_nees))
        print(f"    Approx NEES (position) - mean: {mean_nees:.3f}, median: {median_nees:.3f}")
    ...
    result_data = {
        ...
        'residual': residuals,
        'kf_p_var': kf_p_var,
        'innovation_cov': innovation_covs,
